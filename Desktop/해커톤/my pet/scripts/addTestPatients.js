/**
 * 테스트 병원에 환자 데이터 추가 스크립트
 * 사용법: node scripts/addTestPatients.js
 */

import { initializeApp } from 'firebase/app';
import { getFirestore, collection, doc, setDoc, addDoc, getDocs, getDoc, query, where, serverTimestamp } from 'firebase/firestore';
...
async function addTestPatients() {
  try {
    ...
    const userDoc = await getDocs(query(collection(db, 'users'), where('email', '==', TEST_EMAIL)));
    if (userDoc.empty) {
